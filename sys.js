...
document.getElementById("clearBtn").addEventListener("click", () => {
  ...
  document.getElementById("resultWait")?.classList.remove("hidden");
  parseStatus.classList.add("hidden");
  lookQuestFinished = false;
  mutedQuestKey = "";
  lastQuestKey = "";
  updateQuest();
});
...
function addFiles(fileList) {
  ...
  Array.from(fileList || []).forEach((file) => {
    if (/\.doc$/i.test(file.name) && !/\.docx$/i.test(file.name)) {
      alert("請把 Word（.doc）另存成 .docx 後再上傳，或改用複製貼上。");
      return;
    }
    if (!allowed.test(file.name)) {
      alert(`尚不支援這個檔案：${file.name}\n請改傳 PDF、Word(.docx) 或 Excel。`);
      return;
    }
...
function updateScopeSummary() {
  const labels = {
    auto: "已用報名單位找。名單有人也不會先用學員名字。找不到再改這個。",
    either: "報名單位或學員名字對到其中一種就會出來。",
    club: "已用報名單位找。名單有人也不會先用學員名字。找不到再改這個。",
    names: "只用學員名字，不管報名單位。",
    all: "列出檔案裡所有選手。"
  };
...
function applyViewFilter(updateStatus, forceAll) {
  ...
  document.getElementById("resultWait")?.classList.add("hidden");
  updateStats();
  render();
...
function updateResultNext() {
  ...
  if (shouldGuideToNames()) {
    ...
    box.innerHTML = `
      <div>
        <strong>下一步：改找人方式</strong>
        <p>檔案裡有資料，但用報名單位沒對到人。校隊常用學校名報名，請改選「學員名字」再對一次。</p>
      </div>
      <button type="button" class="btn primary" id="resultNextFindBtn">去改找人方式</button>
    `;
  ...
  box.innerHTML = `
    <div>
      <strong>表已經出來了</strong>
      <p>先看出場順序。看完請匯出存檔。網頁不會保存。找不到人，可改找人方式。</p>
    </div>
    <div class="result-next-actions">
      <button type="button" class="btn export" id="resultNextExportBtn">匯出 Excel</button>
      <button type="button" class="btn gray" id="resultNextFindBtn">改找人方式</button>
    </div>
  `;
...
function viewStatusMessage() {
  ...
  if (mode === "names") {
    if (!getPlayerNames().length) return `學員名單還是空的，所以沒人可顯示。請先填名單，或改選「報名單位」。`;
    if (!shown) return `檔案裡有 ${total} 筆，但學員名字沒對到人。可改選「報名單位」或檢查姓名。`;
    return `完成：檔案裡共 ${total} 筆，只顯示學員名字裡的 ${shown} 筆。`;
  }
  if (mode === "auto") {
    if (lastFilterSource === "namesFallback") {
      return `報名單位沒抓到人，已改用學員名字，顯示 ${shown} 筆（檔案共 ${total} 筆）。建議到下面確認名單。`;
    }
    if (!shown) {
      return getPlayerNames().length
        ? `檔案裡有 ${total} 筆，報名單位和學員名字都沒對到人。`
        : `檔案裡有 ${total} 筆，報名單位沒對到人。可改選「學員名字」再整理。`;
    }
    return `完成：檔案裡共 ${total} 筆，依報名單位顯示 ${shown} 筆。`;
  }
  if (mode === "either") {
    if (!shown) return `檔案裡有 ${total} 筆，報名單位和學員名字都沒對到人。`;
    return `完成：檔案裡共 ${total} 筆，報名單位或學員名字對到 ${shown} 筆。`;
  }
  if (!shown) return `檔案裡有 ${total} 筆，但用館別關鍵字沒對到人。可改選「學員名字」，或檢查關鍵字與姓名。`;
...
function waveLabel(item) {
  ...
  return w === 999999 ? "—" : String(w).padStart(2, "0");
}
  ...
  if (emptyTitle) {
    if (parsedAll.length && allData.length === 0 && mode === "names") {
      emptyTitle.textContent = getPlayerNames().length
        ? "檔案裡找不到學員名單上的人"
        : "學員名單還是空的，請先填人或改選「報名單位」";
    } else if (parsedAll.length && allData.length === 0 && mode === "auto") {
      emptyTitle.textContent = getPlayerNames().length
        ? "報名單位和學員名字都沒對到人"
        : "報名單位沒找到人，可改選「學員名字」再整理";
    } else if (showNameHint) {
      emptyTitle.textContent = "用報名單位沒找到人";
...
const syncToTopBtn = () => {
  toTopBtn?.classList.toggle("is-on", window.scrollY > 280);
};
...
let lastQuestKey = "";
let mutedQuestKey = "";
let lookQuestFinished = false;

function questInputReady() {
  return uploadedFiles.length > 0 || Boolean(sourceData?.value?.trim());
}

function currentQuest() {
  const tourOpen = !document.getElementById("tourModal")?.classList.contains("hidden");
  const onCustom = !document.getElementById("customApp")?.classList.contains("hidden");
  if (tourOpen || onCustom) return null;
  if (!parsedAll.length && !questInputReady()) {
    return { key: "file", how: 0, step: "1", text: "選檔案", target: dropZone, jump: "stepFiles", badge: "點這裡" };
  }
  if (!parsedAll.length && questInputReady()) {
    return { key: "run", how: 1, step: "2", text: "按整理", target: analyzeBtn, jump: "stepRun", badge: "接著按" };
  }
  if (shouldGuideToNames()) {
    return { key: "find", how: 2, step: "!", text: "改找人", target: document.getElementById("resultNext") || document.getElementById("stepWho"), jump: "resultArea", badge: "改找人" };
  }
  if (lookQuestFinished) {
    return { key: "done", how: 3, step: "", text: "", target: null, jump: "", badge: "" };
  }
  return { key: "look", how: 2, step: "3", text: "看出場表", target: document.getElementById("resultNext") || document.getElementById("exportExcelBtn"), jump: "resultArea", badge: "看這裡" };
}
...
function showQuestToast(text) {
  ...
  window.clearTimeout(showQuestToast._timer);
  showQuestToast._timer = window.setTimeout(() => el.classList.remove("is-on"), 2400);
}

function setQuestTarget(el, badgeText) {
  document.querySelectorAll(".quest-now").forEach((node) => node.classList.remove("quest-now"));
  document.querySelectorAll(".quest-badge").forEach((node) => node.remove());
  if (!el) return;
  el.classList.add("quest-now");
  if (!badgeText) return;
...
function updateQuest() {
  ...
  document.querySelectorAll(".how-guide li").forEach((li) => {
    ...
    const done = Boolean(quest && (quest.key === "done" || quest.how > index));
    li.classList.toggle("is-now", now);
    li.classList.toggle("is-done", done);
    const go = li.querySelector(".how-go");
    if (go) go.textContent = now ? "現在做這步" : done ? "完成" : "跳到這一步";
  });

  const hud = document.getElementById("coachHud");
  const hideChrome = !quest || quest.key === "done" || mutedQuestKey === quest.key;
  if (!quest || quest.key === "done") {
    setQuestTarget(null);
    hud?.classList.add("hidden");
    placeQuestPointer(null);
    return;
  }

  setQuestTarget(quest.target, quest.badge);
  const title = document.getElementById("coachHudTitle");
  const text = document.getElementById("coachHudText");
  if (title) title.textContent = quest.step;
  if (text) text.textContent = quest.text;
  hud?.classList.toggle("hidden", hideChrome);
  placeQuestPointer(hideChrome ? null : quest.target);

  if (lastQuestKey && lastQuestKey !== quest.key) {
    const tips = {
      run: "檔案已選好，已幫你滑到整理賽程",
      look: "表出來了，已幫你滑到出場表",
      find: "沒對到人，已幫你滑到提示"
    };
    if (quest.jump) jumpToStep(quest.jump);
    if (tips[quest.key]) showQuestToast(tips[quest.key]);
    if (quest.key === "look") {
      window.clearTimeout(updateQuest._lookTimer);
      updateQuest._lookTimer = window.setTimeout(() => {
        lookQuestFinished = true;
        mutedQuestKey = "look";
        updateQuest();
      }, 2800);
    }
  }
  lastQuestKey = quest.key;
}

document.getElementById("coachHudGo")?.addEventListener("click", () => {
  const quest = currentQuest();
  if (quest?.jump) jumpToStep(quest.jump);
});
document.getElementById("coachHudClose")?.addEventListener("click", (event) => {
  event.stopPropagation();
  mutedQuestKey = lastQuestKey;
  document.getElementById("coachHud")?.classList.add("hidden");
  placeQuestPointer(null);
  showQuestToast("提示已關掉。上面 1 2 3 還可以點");
});
document.getElementById("questPointer")?.addEventListener("click", () => {
  const quest = currentQuest();
  if (quest?.jump) jumpToStep(quest.jump);
});
window.addEventListener("scroll", () => {
  const quest = currentQuest();
  if (quest && quest.target && mutedQuestKey !== quest.key && quest.key !== "done") {
    placeQuestPointer(quest.target);
  }
}, { passive: true });
window.addEventListener("resize", () => {
  const quest = currentQuest();
  if (quest?.target) placeQuestPointer(quest.target);
});
sourceData?.addEventListener("input", updateQuest);
updateQuest();
...
function renderMobileCard(item, index) {
  ...
  const colorLabel = isFight(item) ? "護具" : "出場";
  ...
  const kpis = isOrderStyle(item)
    ? `<div class="card-kpis">
        ...
        <div class="kpi"><span>出場方式</span><strong>${escapeHTML(orderLabel(item))}</strong></div>
      </div>
      <p class="card-note">品勢輪流出場，一個個上場打分，沒有對打、也不看青紅。</p>`
...
const TOUR_SEEN_KEY = "tkd_sys_tutorial_seen_v9";
const TOUR_STEPS = [
  {
    title: "平常只要三步",
    copy: `
    ...
    stage: `
      <ol class="demo-steps">
        <li><span>1</span><div><b>點選檔案</b><small>秩序冊、對打表、品勢表都可以</small></div></li>
        <li><span>2</span><div><b>按整理賽程</b><small>不按，下面不會出表</small></div></li>
        <li><span>3</span><div><b>看出場表</b><small>看完請匯出。找不到人再改找誰</small></div></li>
      </ol>
    `
  },
  {
      ...
      <div class="demo-results">
        <div class="demo-mini-card">
          ...
          <div class="demo-mini-kpis">
            ...
            <div><span>出場方式</span><strong>輪流出場</strong></div>
          </div>
        </div>
        <div class="demo-mini-card">
          <div class="demo-mini-head"><b>黃宥豪</b><span class="type fight">對打</span></div>
          <div class="demo-mini-kpis">
      ...
      <div class="demo-swap">
        <div class="demo-swap-scene a">
          <div class="demo-card on demo-card-unit">
            <b>報名單位</b>
            <small class="demo-ok">有抓到人 → 直接用這個</small>
          </div>
        ...
        <div class="demo-swap-scene b">
          <div class="demo-card dim">
            <b>報名單位</b>
            <small class="demo-miss">沒找到</small>
          </div>
          <div class="demo-arrow">↓</div>
          <div class="demo-card on demo-pulse demo-card-names">
            <b>學員名字</b>
            <small class="demo-ok">改選這個，再套用名單</small>
          </div>
        </div>
      </div>
    `
...
function renderTour() {
  ...
  if (next) next.textContent = tourIndex === TOUR_STEPS.length - 1 ? "開始去做" : "下一個任務";
}
...
function closeTour() {
  ...
  updateQuest();
  showQuestToast("跟著橘色標籤走，下一步會自動滑過去");
}
...
function openExportModal(kind) {
  ...
  if (sheetsNote) sheetsNote.textContent = exportKind === "word" ? "點開可先看出場表" : "檔案裡本來就有，這裡只是先看";
...
function exportSummaryText() {
  ...
  if (both) return `會匯出 ${n} 筆。品勢、對打各一張詳細表，另有簡表。看完再按下載。`;
  if (exportScope === "品勢") return `只出品勢，共 ${n} 筆，做成出場表。`;
  if (exportScope === "對打") return `只出對打，共 ${n} 筆，做成出場表。`;
  return `會匯出 ${n} 筆，做成出場表。看完再按下載。`;
}
...
function renderExportTabs() {
  ...
  const items = exportKind === "word"
    ? [
        ...
        { id: "groups", label: "組別名單" }
      ]
    : [
        { id: "all", label: exportScope === "both" ? "詳細資料（分開）" : "詳細資料" },
  ...
  if (note) {
    const notes = {
      all: exportKind === "word"
        ? (exportScope === "both" ? "會把品勢、對打做成兩張出場表，不是混在同一張。" : "做成出場表。")
        : (exportScope === "both"
...
function renderExportPreview() {
  ...
  if (tab === "all") {
    if (!selected.length) {
      host.innerHTML = `<p class="export-preview-empty">請先勾選要匯出的欄位</p>`;
      return;
    }
...
function previewDetailBlocks(club, headers, rowFn, kind) {
  ...
  if (exportScope !== "both") {
    ...
    return previewTableHtml(headers, rowFn(exportItems()), `${club}｜${kind}${label ? "-" + label : ""}（詳細表）`);
  }
  const parts = [];
  if (poom.length) parts.push(previewTableHtml(headers, rowFn(poom), `${club}｜${kind}-品勢（詳細表）`));
  if (fight.length) parts.push(previewTableHtml(headers, rowFn(fight), `${club}｜${kind}-對打（詳細表）`));
...
function previewTableHtml(headers, rows, caption) {
  ...
  const extra = rows.length > shown.length ? `，後面還有 ${rows.length - shown.length} 筆沒顯示` : "";
  return `
    <p class="export-preview-meta">${escapeHTML(caption)}｜先看 ${shown.length} 筆／共 ${rows.length} 筆${extra}</p>
    <div class="export-preview-wrap">
      <table class="export-preview-table">
        <thead><tr>${headers.map((h) => `<th>${escapeHTML(h)}</th>`).join("")}</tr></thead>
        <tbody>
          ${shown.map((row) => `<tr>${row.map((cell) => `<td>${escapeHTML(String(cell ?? ""))}</td>`).join("")}</tr>`).join("") || `<tr><td colspan="${headers.length}">沒有資料</td></tr>`}
        </tbody>
      </table>
    </div>
  `;
...
function wordGroupsPreviewHtml() {
  ...
  const extra = groups.length > shown.length ? `<p class="export-preview-meta">下面還有 ${groups.length - shown.length} 組沒顯示</p>` : "";
  return `
    <p class="export-preview-meta">Word 組別名單｜先看 ${shown.length} 組／共 ${groups.length} 組</p>
    <div class="export-preview-wrap">
      <div class="export-preview-groups">
        ${shown.map((group) => {
          const people = (group.members.length ? group.members : group.ours).slice(0, 8).map((item) =>
            `<li>${escapeHTML(displayValue(item.player))}（${escapeHTML(displayValue(item.club))}）</li>`
          ).join("");
          return `<div><h4>${escapeHTML(displayValue(group.type))} ｜ ${escapeHTML(displayValue(group.division))}</h4><ul>${people || "<li>未提及</li>"}</ul></div>`;
        }).join("") || `<p class="export-preview-empty">沒有組別</p>`}
      </div>
    </div>
    ${extra}
  `;
...
function doExportExcel() {
  ...
  if (exportScope === "both" && poomsae.length && fight.length) {
    addDetailSheet("詳細資料-品勢", poomsae);
    addDetailSheet("詳細資料-對打", fight);
  } else {
...
function doExportWord() {
  ...
  const tables = exportScope === "both" && poom.length && fight.length
    ? `<h2>品勢（詳細表）</h2><table>${tableHead}${wordTableRows(poom)}</table>
        <h2>對打（詳細表）</h2><table>${tableHead}${wordTableRows(fight)}</table>`
...
function rosterChip(name, checked) {
  return `
    <label class="check-chip">
      ...
      <button type="button" class="chip-del" data-name="${escapeHTML(name)}" aria-label="從常用名單刪除 ${escapeHTML(name)}">×</button>
    </label>
  `;
}
...
function renderRosterList() {
  ...
  box.innerHTML = names.map((name) => rosterChip(name, checked.has(name))).join("")
    || `<p class="hint">還沒有名單。可在下面新增，或先用預設的常用名單。</p>`;
  filterRosterList();
}
...
function resetRosterToDefault() {
  if (!confirm("要把常用名單還原成金城土城預設名單嗎？你自己加的人會被清掉。")) return;
  clubRoster = DEFAULT_CLUB_ROSTER.slice();
